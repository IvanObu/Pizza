...
class Toppings(models.Model):
    ...
    top_category = models.CharField(
        choices=TopCategory.choices, max_length=2, verbose_name="категория топпинга"
    )
...
class DrinkSize(models.Model):
    ...
    drink = models.ForeignKey(Drink, on_delete=models.CASCADE, related_name="variants")
...
class Pizza(models.Model):
    ...
    image = models.ImageField(
        upload_to="Pizza/%Y/%m/%d", blank=True, null=True, verbose_name="Изображение"
    )
    category = models.ForeignKey(
        Category,
        on_delete=models.CASCADE,
    ...
    toppings = models.ManyToManyField(Toppings, blank=True, verbose_name="Ингредиенты")
    ...
    weight_multiplier_m = models.DecimalField(
        max_digits=4, decimal_places=2, default=1.3, verbose_name="Коэф. веса M"
    )
    weight_multiplier_l = models.DecimalField(
        max_digits=4, decimal_places=2, default=1.6, verbose_name="Коэф. веса L"
    )
    weight_multiplier_xl = models.DecimalField(
        max_digits=4, decimal_places=2, default=2.0, verbose_name="Коэф. веса XL"
    )
    ...
    price_xl = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="Цена XL"
    )
    ...
    weight_xl = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="Вес XL"
    )
...
class Combo(models.Model):
    ...
    roman_pizzas = models.ManyToManyField(
        RomaPizza, through="ComboRomaPizza", blank=True
    )
    ...
    def get_items_price(self):
        ...
        for item in self.combodrink_set.select_related(
            "drink_size", "drink_size__drink"
        ):
...
class ActionGallery(models.Model):
    """Заглушка для админки, чтобы использовать inline"""
    title = models.CharField(
        "Название галереи", 
        max_length=100, 
        default="Галерея акций"
    )
    
    class Meta:
        verbose_name = "Галерея акций"
        verbose_name_plural = "Галереи акций"
...
class ActionImage(models.Model):
    ...
    image = models.ImageField(
        upload_to="Actions/%Y/%m/%d", blank=True, null=True, verbose_name="Изображение"
    )
